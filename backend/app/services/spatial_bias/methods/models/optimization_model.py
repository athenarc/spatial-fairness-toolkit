...
from app.services.spatial_bias.methods.models.abstract_model import (
    SpatialFairnessModel,
)
import numpy as np
from app.services.spatial_bias.utils.thresholds_utils import (
    adjusted_thresholds,
    convert_indiv_sol_2_regions_sol,
)
from app.services.spatial_bias.methods.optimization.promis_opt import (
    minimize_promis_opt_obj,
    minimize_promis_opt_obj_overlap,
)
from app.services.spatial_bias.methods.optimization.promis_app import (
...
class SpatialOptimFairnessModel(SpatialFairnessModel):
    ...
    def multi_fit(
        ...
        Args:
            ...
            budget_range (list): List of budgets (number of flips) to iterate over.
        ...
        for budget in budget_range:
            ...
            self.budget_to_solution_info[budget] = {
    ...
    def fit(
        ...
        Args:
            ...
            budget (int): Number of flips allowed.
        ...
        assert fair_notion in [
            "statistical_parity",
            "equal_opportunity",
        ], f"Invalid fair notion: {fair_notion}, should be one of: ['statistical_parity', 'equal_opportunity']"
        if fair_notion == "equal_opportunity" and y_true is None:
            raise ValueError("For Equal Opportunity the <y_true> should be provided")

        y_pred = np.array(y_pred)
        y_pred_probs = np.array(y_pred_probs) if y_pred_probs is not None else None
        ...
        self.st_par = True if fair_notion == "statistical_parity" else False
        if fair_notion == "equal_opportunity":
            ...
            y_pred_probs = (
                y_pred_probs[pos_y_true_indices] if y_pred_probs is not None else None
            )
        ...
        self.overlap = (
            self.is_overlapping(points_per_region) if overlap is None else overlap
        )
        self.val_n_s = (
            [len(pts) for pts in points_per_region] if self.overlap is True else None
        )
        ...
        if budget <= 1:
            budget = np.round(budget * len(y_pred))
        ...
        (
            ...
            optim_method_func=self.optim_method_func_dict[(self.method, self.overlap)],
        ...
        if status in [1, 3]:  # 1 for optimal solution, 3 for work limit reached
            self.fitted = 1
    ...
    def predict(self, points_per_region, y_pred, apply_fit_flips=False):
        """
        Generates spatial fairness predictions for different budget levels.
        ...
        if apply_fit_flips:
            ...
            y_pred_new[self.pts_to_change] = (
                y_pred_new[self.pts_to_change] + self.pts_to_change_sol
            )
            return y_pred_new
    ...
    def is_overlapping(self, points_per_region):
        """
        Check if the regions defined by points_per_region are overlapping.

        Args:
            points_per_region (list): List of points per region.

        Returns:
            bool: True if regions are overlapping, False otherwise.
        """
        seen_points = set()
        for region in points_per_region:
            for point in region:
    ...
    def _convert_region_sol_2_int_indiv_sol(self, dp_s, points_per_region, y_pred):
        ...
        for idx, dp in enumerate(dp_s):
            # int_dp = int(round(dp))
            int_dp = int(dp)
            decimal_dp = dp - int_dp

            # if the region has no flips then continue
            if int_dp != 0 or decimal_dp != 0:
                ...
                if decimal_dp != 0:
                    should_flip = np.random.rand() < np.abs(decimal_dp)
                    if should_flip:
                        nxt_pt_to_change_idx = all_pts_to_change_indices[np.abs(int_dp)]
                        sol[reg_pts[nxt_pt_to_change_idx]] = flip_dir
    ...
    def _get_optimization_solutions(
        ...
        sol, status, obj_val = optim_method_func(**args, C=budget)
        new_thresholds = None
    ...
    def get_model_fit_info_as_df(self):
        ...
        data = []
        for budget, info in self.budget_to_solution_info.items():
            data.append(
                {
                    "budget": budget,
